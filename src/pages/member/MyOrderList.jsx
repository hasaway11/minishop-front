...
function OrderElement({order}) {
  ...
  return (
    <>
      {status==='COMPLETE' && <Alert variant="seconary">배송완료 ({orderTime})</Alert>}
      {status==='SHIPPING' && <Alert variant="primary">배송중 {orderTime} 주문</Alert>}
      {status==='PAY' && <Alert variant="success">결제완료 : {orderTime} 주문</Alert>}
